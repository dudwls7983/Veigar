...
namespace Veigar
{
    ...
    {
        static event KeyboardHooker.HookedKeyboardUserEventHandler _proc;
        
        private static long SetHook(int iKeyWhatHappened, int vkCode)
        {
            // if return 1, prevent the key
            long lResult = 0;
            
            if (vkCode == 115 && iKeyWhatHappened == 32) //scanning key ALT + F4
            {
                // find the league of legends
                if(GetForegroundProcessName().Equals("League of Legends"))
                {
                    // close league of legends
                    Process[] processLists = Process.GetProcessesByName("League of Legends");
                    if (processLists.Length > 0)
                    {
                        processLists[0].Kill();

                        // 강제종료된 프로세스 이후 포커싱 될 프로세스가 꺼지는 것을 방지
                        lResult = 1;
                    }
                }
            }
